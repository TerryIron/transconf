...
cmd_conf = as_config(os.path.join(os.path.dirname(__file__),
                                  '{0}/agent_cmds.ini'.format(CONF_PATH)))
...
class AgentMiddleware(TranMiddleware):
    def process_request(self, context):
        return super(AgentMiddleware, self).process_request(context)


if __name__ == '__main__':
    command_configure(cmd_conf)
    app = loadapp('config:agent.ini',
                  'main',
